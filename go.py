'''
This program is to define the rule and situation of the Go Board.
...
ALL_COORDS = []
EMPTY_BOARD = None
NEIGHBORS = {}  # Denote the neighbors of each legal position on the board
DIAGONALS = {}  # Same as above: (0, 1) : neighbors: {(0, 1): [(0, 0), (0, 2), (1, 1)]}
...
class LibertyTracker():
    """
    This class mainly deal with the liberty in Go.
    """
    # TODO: to be more clear about what these functions in LibertyTracker for
    @staticmethod
...
class Position():
    """
    This class is mainly about the relative properties of a position on the Go board.
    Including: simulate the situation when agent plays a move on the board.
               whether a move is legal, is suicidal, is ko and so on.
               calculate the final score of Black and White.
    """
    def __init__(self, board=None, n=0, komi=7.5, caps=(0, 0), lib_tracker=None, ko=None, recent=tuple(), to_play=BLACK):
        ...
        self.komi = komi
        self.caps = caps
        self.lib_tracker = lib_tracker or LibertyTracker.from_board(self.board) # assigned the second one if lib_tracker is none
        self.ko = ko
        self.recent = recent
    ...
    def play_move(self, c, color=None, mutate=False):
        ...
        # place a stone, if some stones are captured, lift the stone
        # out of the board.
